...
export function UploadShoe({ onClose }) {
  ...
  const [previewImage, setPreviewImage] = React.useState(() => getCachedGameImage("car"));
  ...
  const handleContinue = async () => {
    ...
    try {
      setIsSaving(true);
      if (pendingImage) {
        await saveGameImage("car", pendingImage);
      }
      navigate(GAME_IMAGE_CONFIG.car.route, { state: { uploadedImage: imageToUse } });
    } catch (error) {
  ...
  useEffect(() => {
    ...
    const ensureSession = async () => {
      ...
      try {
        ...
        console.error("Failed to start car analytics session:", error);
      }
    };
...
opacity:"0.9",
          ...
          <Box component='img' sx={{ width: { lg: "450.59px",sm:"50%" }, height: {lg:"390.96px",sm:"52vh"}, marginTop: "-8px", marginLeft: {lg:"150px",sm:"-3%"}, borderRadius: "200.58px", objectFit: "contain", transform: isLionSpeaking ? "none" : "translateY(18px) scale(1.05, 1.02)", transformOrigin: "center" }} src={isLionSpeaking ? newgif : standinglion} />
        </Box>
                  ...
                  >
                    ...
                    >
                      {audioFinished
                        ...
                        ? "Verified: car ✅"
                        : i18n.language === "ur"
                          ? `آگے جانے کے لیے گاڑی بولیں (${speechStep}/2)`
