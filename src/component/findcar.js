...
useEffect(() => {
  ...
  useEffect(() => {
    ...
    const ensureSession = async () => {
      if (window.sessionStorage.getItem("analytics:wonderworld:car")) return;
      ...
      try {
        ...
        if (!ignore) {
          ensureWonderworldSessionState("car", sessionId, i18n.language);
        }
      } catch (error) {
        console.error("Failed to ensure car analytics session:", error);
      }
    };
  ...
  useEffect(() => {
    updateWonderworldEmotionCounts("car", emotionCounts);
  }, [emotionCounts]);

  useEffect(() => {
    updateWonderworldFocusMetrics("car", getMetrics());
      ...
      <Box sx={{ cursor: `url(${click}) 122 122, auto` }}>
        ...
        >
          ...
          <Box
            ...
            sx={{ position: "absolute", top: "5cqh", left: "5cqw", zIndex: 10, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", "&:hover": { transform: "scale(1.08)", transition: "0.2s" } }}
          >
          ...
          <Box sx={{ position: "absolute", bottom: favoriteCharacter === "bubbles" || favoriteCharacter === "mimmi" || favoriteCharacter === "mimi" ? "9cqh" : "16cqh", left: "3cqw", width: favoriteCharacter === "bubbles" || favoriteCharacter === "mimmi" || favoriteCharacter === "mimi" ? "max(32cqw, 48cqh)" : "max(26cqw, 39cqh)", zIndex: 5, "@media (max-aspect-ratio: 1.55)": { left: "-1cqw" }, "@media (min-aspect-ratio: 1.55)": { bottom: favoriteCharacter === "bubbles" || favoriteCharacter === "mimmi" || favoriteCharacter === "mimi" ? "11cqh" : "16cqh" }, "@media (min-width: 1000px) and (max-width: 1160px) and (max-height: 780px)": { bottom: favoriteCharacter === "bubbles" || favoriteCharacter === "mimmi" || favoriteCharacter === "mimi" ? "10cqh" : "16cqh" } }}>
            <Box sx={{ position: "absolute", width: "max(20cqw, 30cqh)", height: "auto", bottom: "88%", left: "50%", zIndex: 6, "@media (max-aspect-ratio: 4/3)": { width: "22cqw", left: "40%" } }}>
              ...
              <Typography sx={{ fontSize: i18n.language === "ur" ? "max(3.2cqw, 4.8cqh)" : "max(2.2cqw, 3.3cqh)", position: "absolute", top: "54%", left: "52%", transform: "translate(-50%, -70%)", width: "75%", textAlign: "left", fontStyle: "normal", lineHeight: "1.35", fontFamily: i18n.language === "ur" ? "JameelNooriNastaleeq" : "Chewy", letterSpacing: "1px", color: "#fff", opacity: "0.9" }}>
                {t("questioncar")}
              </Typography>
            </Box>
          ...
          <Box sx={{ position: "absolute", right: "2cqw", bottom: "16cqh", width: "max(55cqw, 82cqh)", aspectRatio: "1024 / 796", "@media (max-aspect-ratio: 1.55)": { width: "max(65cqw, 92cqh)", right: "1cqw", bottom: "14cqh" }, "@media (min-width: 1160px) and (max-width: 1250px) and (min-height: 800px) and (max-height: 900px)": { bottom: "12cqh" }, "@media (min-width: 1000px) and (max-width: 1160px) and (max-height: 780px)": { bottom: "14cqh" }, "@media (min-width: 1300px) and (max-aspect-ratio: 1.4)": { width: "max(55cqw, 82cqh)", right: "2cqw", bottom: "12cqh" }, containerType: "size", zIndex: 4 }}>
            ...
            <Box component="img" src={car} onClick={() => handleSelect(car)} sx={{ position: "absolute", left: "16.5%", top: "21%", width: "20%", height: "auto", cursor: "pointer", zIndex: 5, "&:hover": { transform: "scale(1.12)" } }} />
            <Box component="img" src={ball} onClick={() => handleSelect(ball)} sx={{ position: "absolute", left: "45%", top: "24%", width: "14.5%", height: "auto", cursor: "pointer", zIndex: 5, "&:hover": { transform: "scale(1.12)" } }} />
            <Box component="img" src={shoe} onClick={() => handleSelect(shoe)} sx={{ position: "absolute", left: "68.2%", top: "21%", width: "19.5%", height: "auto", cursor: "pointer", zIndex: 5, "&:hover": { transform: "scale(1.12)" } }} />

            {[
              { text: t("car"), left: "26.5%" },
              { text: t("ball"), left: "51%" },
              { text: t("shoes"), left: "78%" },
            ].map((item) => (
            ...
            {selectedImageSrc && (
              <Box component="img" src={selectedImageSrc} sx={{ position: "absolute", left: "53%", top: "74%", width: selectedImageSrc === ball ? "13%" : "16%", height: "auto", transform: "translate(-50%, -50%)", objectFit: "contain", zIndex: 6 }} />
            )}
          </Box>
          ...
          <audio ref={audioRef} src={i18n.language === "ur" ? findurdu : findcar} preload="auto" />
          ...
          {cameraAllowed && (
            <video ref={videoRef} autoPlay muted playsInline style={{ position: "absolute", width: "1px", height: "1px", opacity: 0, pointerEvents: "none" }} />
          )}
        </Box>
      </Box>
    </motion.div>
