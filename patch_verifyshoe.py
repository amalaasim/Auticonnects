...
# Refactor the Verifyshoe internal layout to be strictly absolute
old_verify_start = text.find('      <Box sx={{ \n        position: "relative", \n        width: "max(65cqw, 80cqh)",')
old_verify_end = text.find('      </Box>\n    </Box>\n  );\n\n  return(')

if old_verify_start != -1 and old_verify_end != -1:
    new_verify = """      <Box sx={{ 
        position: "relative", 
        width: "max(65cqw, 80cqh)",
        maxWidth: "80vw", 
    ...
    text = text[:old_verify_start] + new_verify + text[old_verify_end:]
    
    with open("src/component/learnobjshoe.js", "w") as f:
        f.write(text)
    print("Patched VerifyShoe!")
else:
    print("Could not find VerifyShoe boundaries.")
